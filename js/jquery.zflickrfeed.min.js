...
var desc=data.description;if(desc=='')desc=data.title;html+='<div class="flickrHeader">'+'<a href="'+data.link+'" title="'+desc+'">'+data.title+'</a>'+'</div>';html+='<div class="flickrBody">'+'<ul>';var feeds=data.items;var count=feeds.length;if(count>options.limit)count=options.limit;for(var i=0;i<count;i++){var photo=feeds[i];var link='<a href="'+photo.link+'" title="'+photo.title+' (View on Flickr)" target="_blank">';html+='<li class="flickrRow '+row+'">';var src=photo.media.m;if(options.imagesize=='square')src=src.replace('_m','_s');if(options.imagesize=='thumbnail')src=src.replace('_m','_t');if(options.imagesize=='medium')src=src.replace('_m','');html+=link+'<img src="'+src+'" alt="'+photo.title+'" /></a>'
...
// Initiate Flickr Feed...
$(document).ready(function () {
	$('.flickr-stream').flickrfeed( '' , 'winter,christmas' , {
		limit		: 9,		
		header		: false,
		title		: false,
		date		: false,
